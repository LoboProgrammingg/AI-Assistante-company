...
from app.models import User, Message, Reminder, Finance, FinanceType, RecurrenceType, Meeting
from app.ai import WhatsAppAIAgent, MeetingAgent
...
def create_reminder_from_entities(db: Session, user_id: int, entities: dict, user_timezone: str = "America/Cuiaba") -> Reminder:
    ...
    from app.utils.timezone_helper import get_current_time_for_user, convert_to_utc
        ...
        try:
            ...
            if scheduled_time.year == 1900:  # dateutil usa 1900 quando não há ano
                scheduled_time = scheduled_time.replace(
                    year=current_time_user.year,
                    month=current_time_user.month,
                    day=current_time_user.day
                )
...
def create_finance_from_entities(db: Session, user_id: int, entities: dict, timezone: str = "America/Sao_Paulo") -> Finance:
    ...
    category = db.query(FinanceCategory).filter(
        FinanceCategory.name.ilike(f"%{category_name}%"),
        FinanceCategory.type == fin_type
    ).first()
    ...
    logger.info(f"Data da transação: {transaction_date.strftime('%d/%m/%Y')}, Categoria: {category.name if category else 'N/A'}")
    ...
    db.commit()
    db.refresh(finance)
    logger.info(f"Transação criada: {finance.id} - R${finance.amount} - {category.name if category else 'Sem categoria'}")
    return finance
        ...
        if next_action == "create_reminder" and entities.get("reminder"):
            ...
            try:
                for reminder_data in entities["reminders"]:
                    create_reminder_from_entities(db, current_user.id, {"reminder": reminder_data}, current_user.timezone or "America/Cuiaba")
            except Exception as e:
                logger.error(f"Erro ao criar múltiplos lembretes: {e}")
        
        elif next_action == "create_finance" and entities.get("finance"):
            try:
                create_finance_from_entities(db, current_user.id, entities, current_user.timezone or "America/Sao_Paulo")
            except Exception as e:
            ...
            try:
                for finance_data in entities["finances"]:
                    create_finance_from_entities(db, current_user.id, {"finance": finance_data}, current_user.timezone or "America/Sao_Paulo")
...
def is_meeting_audio(transcription: str, file_size: int) -> bool:
    ...
    meeting_keywords = [
        "reunião", "meeting", "participantes", "pauta", "agenda",
        "ata", "decisão", "ação", "próximos passos", "discussão",
        "apresentação", "projeto", "equipe", "time", "sprint",
        "finalizar", "integração", "desenvolvimento", "funcionalidade"
    ]
...
async def process_meeting_audio(
    ...
    result = await meeting_agent.process(
        ...
        context={"user_name": user_name, "is_transcription": True}
    )
...
async def send_audio(
    ...
    try:
        ...
        is_valid = (
            ...
            any(filename.lower().endswith(ext) for ext in [".mp3", ".wav", ".ogg", ".webm", ".m4a", ".opus"])
        )
        ...
        try:
            ...
            save_message(
                db=db,
                user_id=current_user.id,
                content=f"[Áudio] {transcription[:200]}...",
                direction="incoming"
            )
