...
from app.api.deps import create_access_token, get_current_user, get_db
from app.models import User
from app.schemas import UserCreate, UserResponse, UserStats, UserUpdate
from app.services import FinanceService, MeetingService, ReminderService, cache_service
...
def create_user(data: UserCreate, db: Session = Depends(get_db)):
    ...
    if existing:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Usuário já existe com este número")
...
def login(data: dict, db: Session = Depends(get_db)):
    ...
    if not phone_number:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="phone_number é obrigatório")
...
def update_current_user(
    data: UserUpdate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)
):
...
def generate_token(current_user: User = Depends(get_current_user)):
