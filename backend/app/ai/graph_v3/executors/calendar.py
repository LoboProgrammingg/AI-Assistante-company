    ...
    def create_event(params: Dict, db: Any, user_id: int, user_name: str) -> ExecutionResult:
        ...
        try:
            ...
            try:
                ...
                return ExecutionResult(
                    success=False, action_type="create_event", error=f"Formato de data inválido: {data_hora}"
                )
            ...
            return ExecutionResult(
                success=False, action_type="create_event", error=result.get("error", "Erro ao criar evento")
            )

        except ImportError:
            return ExecutionResult(
                success=False, action_type="create_event", error="Serviço de Calendar não disponível"
            )
    ...
    def list_events(params: Dict, db: Any, user_id: int, user_name: str) -> ExecutionResult:
        ...
        try:
            ...
            if result.get("success"):
                ...
                return ExecutionResult(
                    success=True, action_type="list_events", data={"events": events}, response_template=template
                )
    ...
    def check_availability(params: Dict, db: Any, user_id: int, user_name: str) -> ExecutionResult:
        ...
        try:
            ...
            if result.get("success"):
                ...
                return ExecutionResult(
                    success=True, action_type="check_availability", data=result, response_template=template
                )
