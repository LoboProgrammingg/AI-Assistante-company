...
class ReminderExecutor:
    ...
    def delete(params: Dict, db: Any, user_id: int, user_name: str) -> ExecutionResult:
        ...
        try:
            ...
            return ExecutionResult(success=count > 0, action_type="delete_reminder", data=result, response_template=template)
        except Exception as e:
    ...
    def update(params: Dict, db: Any, user_id: int, user_name: str) -> ExecutionResult:
        ...
        try:
            ...
            if result.get("success"):
                return ExecutionResult(success=True, action_type="update_reminder", data=result, response_template="✏️ Lembrete atualizado!")
            
            return ExecutionResult(success=False, action_type="update_reminder", error=result.get("error", "Não encontrado"))
        except Exception as e:
