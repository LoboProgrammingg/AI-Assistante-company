...
class WorkingContextBuilder:
    ...
    def build(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(
            f"[CONTEXT_BUILDER] memories={len(memories)} → "
            f"filtered={len(filtered)} → context={len(context)} chars"
        )
    ...
    def _filter_allowed(self, memories: List[MemoryItem]) -> List[MemoryItem]:
        ...
        return [
            m for m in memories
            if m.memory_type not in NEVER_TO_LLM_TYPES
        ]
    ...
    def _format_context(self, memories: List[MemoryItem], user_name: str = "") -> str:
        ...
        habits_recurrence = (
            by_type.get(MemoryType.HABIT, [])[:2] +
            by_type.get(MemoryType.RECURRENCE, [])[:2]
        )
