...
class MemoryManager:
    ...
    def build_context_prompt(self) -> str:
        ...
        other_facts = {k: v for k, v in facts.items() 
                      if k not in ["name", "profissao", "familia", "objetivo", "preferencia"]}
        if other_facts:
            parts.append(f"Outros fatos: {other_facts}")
        
        if stats:
            parts.append(f"Estatísticas: {stats.get('total_messages', 0)} mensagens, "
        ...
        if contacts.get("total", 0) > 0:
            ...
            for group, group_contacts in by_group.items():
                ...
                parts.append(f"  • {group_label}: {', '.join(contact_names)}" + 
        ...
        if recent_actions:
            ...
            for action in recent_actions:
                ...
                if action_type == "create_finance":
                    ...
                    parts.append(f"  ✅ Transação registrada: {finance.get('description', 'N/A')} - R${finance.get('amount', 0)} ({timestamp})")
    ...
    def analyze_user_behavior(self, message: str) -> None:
        ...
        # Linguagem informal
        informal_words = ["vc", "tb", "pq", "blz", "vlw", "flw", "tmj", "kk", "haha", "rsrs", "eae", "ae", "mano", "cara"]
