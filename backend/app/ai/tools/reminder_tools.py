...
class CriarLembreteSchema(BaseModel):
    ...
    titulo: str = Field(
        description="Título ou descrição do lembrete (ex: 'Reunião com cliente')",
        min_length=2,
        max_length=200
    )
    data_hora: str = Field(
    ...
    recorrencia: Optional[Literal["none", "daily", "weekly", "monthly"]] = Field(
        description="Tipo de recorrência: none, daily, weekly ou monthly",
        default="none"
    )
    prioridade: Optional[Literal["low", "medium", "high"]] = Field(
        description="Prioridade do lembrete",
        default="medium"
    )
...
class ListarLembretesSchema(BaseModel):
    ...
    periodo: Literal["hoje", "amanha", "semana", "todos"] = Field(
        description="Período: hoje, amanha, semana ou todos",
        default="semana"
    )
    apenas_pendentes: bool = Field(
        description="Se True, mostra apenas lembretes pendentes",
        default=True
    )
...
class DeletarLembreteSchema(BaseModel):
    ...
    lembrete_id: Optional[int] = Field(
        description="ID do lembrete a deletar",
        default=None
    )
    titulo: Optional[str] = Field(
        description="Título parcial para encontrar o lembrete",
        default=None
    )
