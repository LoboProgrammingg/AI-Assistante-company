...
class ReminderAgent(BaseAgent):
    ...
    def process_sync(self, message: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        extraction_prompt = ReminderPrompts.get_extraction_prompt(
            context=self.format_context(context), current_time=current_time.strftime("%d/%m/%Y %H:%M"), message=message
        )
        ...
        if extracted.get("needs_clarification"):
            response_text = extracted.get("clarification_question", "Poderia me dar mais detalhes sobre o lembrete?")
            return {"response": response_text, "entities": {}, "next_action": "await_clarification", "confidence": 0.0}
        ...
        if reminders_need_time:
            ...
            if len(reminders) > 1:
                titles = "\n".join(
                    [f"• *{r.get('title')}* - {self._format_datetime(r.get('scheduled_time'))}" for r in reminders]
                )
                return {
                    "response": (
    ...
    def _handle_delete_sync(self, message: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        reminders_text = "\n".join(
            [f"ID {r.id}: {r.title} - {r.scheduled_time.strftime('%d/%m/%Y %H:%M')}" for r in reminders]
        )

        identify_prompt = ReminderPrompts.get_delete_identification_prompt(
            message=message, reminders_text=reminders_text
        )
        ...
        options = "\n".join([f"• **{r.title}** - {r.scheduled_time.strftime('%d/%m %H:%M')}" for r in reminders[:5]])
    ...
    def _complete_reminder_with_time_sync(
        self, message: str, pending_reminder: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _build_confirmation_response(self, extracted: Dict[str, Any]) -> Dict[str, Any]:
        ...
        confirmation = ReminderPrompts.TEMPLATES["single_confirmation"].format(
            title=title, scheduled_time=scheduled, remind_time=f"{time_text} antes" if remind_before > 0 else time_text
        )
    ...
    def _build_multiple_confirmation_response(self, reminders: list) -> Dict[str, Any]:
        ...
        confirmation = ReminderPrompts.TEMPLATES["multiple_confirmation"].format(
            count=len(reminders), items=items, remind_time=f"{time_text} antes" if remind_before > 0 else time_text
        )
