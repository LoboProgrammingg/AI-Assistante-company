...
class ConfidenceScorer:
    ...
    ACTION_MULTIPLIERS = {
        ...
        "list_contacts": 1.2,
        "list_events": 1.2,
        "list_todoist_tasks": 1.2,
        
        # Ações de criação - risco médio
        "create_finance": 0.9,
        "create_reminder": 1.0,
        "create_contact": 1.0,
        "create_todoist_task": 1.0,
    ...
    def calculate(
        ...
        requires_confirm = (
            action in cls.ALWAYS_CONFIRM or
            (0.5 <= score < 0.9 and action not in cls.NEVER_AUTO)
        )
        ...
        logger.info(
            f"[CONFIDENCE] {action}: {score:.0%} | "
            f"Auto: {can_auto} | Confirm: {requires_confirm}"
        )
