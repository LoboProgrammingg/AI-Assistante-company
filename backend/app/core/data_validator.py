...
class DataValidator:
            ...
            for i, p in enumerate(participants):
                if isinstance(p, str) and len(p) > 100:
                    results.append(ValidationResult(
                        ...
                        message="Nome de participante muito longo"
                    ))
    ...
    def _validate_amount(self, amount: Any) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=False,
                field="amount",
                message=f"Valor inválido: {amount}"
            )
    ...
    def _validate_category(self, category: str) -> ValidationResult:
        ...
        valid_categories = {
            ...
            "Salário", "Freelance", "Investimentos", "Vendas", "Outros Receitas"
        }
            ...
            for valid in valid_categories:
                if category_normalized.lower() in valid.lower():
                    return ValidationResult(
                        is_valid=True,
                        field="category",
                        message="Categoria corrigida",
                        corrected_value=valid
                    )
    ...
    def _validate_transaction_type(self, trans_type: str) -> ValidationResult:
        ...
        if trans_type.lower() not in valid_types:
            return ValidationResult(
                is_valid=False,
                field="type",
                message=f"Tipo inválido: {trans_type}",
                corrected_value="expense"
            )
        ...
        try:
            if isinstance(dt_value, str):
                ...
                if not parsed:
                    return ValidationResult(
                        is_valid=False,
                        field=field,
                        message=f"Formato de data inválido: {dt_value}"
                    )
                ...
                return ValidationResult(
                    is_valid=False,
                    field=field,
                    message=f"Tipo de data inválido: {type(dt_value)}"
                )
            ...
            if dt_value < min_date:
                return ValidationResult(
                    is_valid=False,
                    field=field,
                    message=f"Data muito antiga: {dt_value}"
                )
            ...
            if dt_value > max_date:
                return ValidationResult(
                    is_valid=False,
                    field=field,
                    message=f"Data muito no futuro: {dt_value}"
                )
            ...
            return ValidationResult(
                is_valid=False,
                field=field,
                message=f"Erro ao validar data: {e}"
            )
    ...
    def _check_reminder_exists(self, reminder_id: int, user_id: int) -> ValidationResult:
        ...
        try:
            ...
            exists = self.db.query(Reminder).filter(
            ...
            if not exists:
                return ValidationResult(
                    is_valid=False,
                    field="reminder_id",
                    message=f"Lembrete #{reminder_id} não encontrado"
                )
