...
class InputSanitizer:
    ...
    def __init__(self, config: SanitizerConfig = None):
        ...
        self._compiled_patterns = [re.compile(p, re.IGNORECASE | re.DOTALL) for p in self.DANGEROUS_PATTERNS]
    ...
    def sanitize_message(self, message: str) -> str:
        ...
        if self.config.log_sanitization and message != original:
            logger.info(f"Mensagem sanitizada: {len(original)} -> {len(message)} chars")
            ...
            if isinstance(value, str):
                ...
                sanitized[key] = [
                    (
                        ...
                        else self.sanitize_field(item) if isinstance(item, str) else item
                    )
                    for item in value
                ]
            else:
