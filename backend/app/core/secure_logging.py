...
class SecureLogger:
    ...
    def _mask_args(self, args: tuple) -> tuple:
        ...
        return tuple(mask_sensitive_data(str(arg)) if isinstance(arg, str) else arg for arg in args)

    def _mask_kwargs(self, kwargs: Dict[str, Any]) -> Dict[str, Any]:
        """Mascara kwargs do log."""
        if "extra" in kwargs and isinstance(kwargs["extra"], dict):
            kwargs["extra"] = {
                k: mask_sensitive_data(str(v)) if isinstance(v, str) else v for k, v in kwargs["extra"].items()
            }
        return kwargs
