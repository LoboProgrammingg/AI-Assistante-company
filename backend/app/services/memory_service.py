...
class MemoryService:
    ...
    def get_user_data_summary(self, user_id: int) -> Dict[str, Any]:
        ...
        from app.models import Finance, Reminder, Meeting, Contact, Document
        ...
        finances_this_month = self.db.query(Finance).filter(
            and_(
        ...
        finances_last_month = self.db.query(Finance).filter(
            and_(
                Finance.user_id == user_id,
        ...
        ).order_by(Reminder.scheduled_time.asc()).limit(5).all()
        ...
        ).order_by(Contact.group_name.asc(), Contact.name.asc()).all()
        ...
        for c in contacts:
            ...
            contacts_by_group[group].append({
                "name": c.name,
    ...
    def clear_user_memory(self, user_id: int) -> int:
        ...
        """
        deleted = self.db.query(ConversationMemory).filter(
            ConversationMemory.user_id == user_id
        ).delete()
