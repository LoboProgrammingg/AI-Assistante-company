...
from app.models import Reminder, RecurrenceType
from app.schemas.reminder import ReminderCreate, ReminderUpdate, ReminderFromAI
...
class ReminderService:
    ...
    def create_from_entities(
        ...
        try:
            scheduled_time = datetime.fromisoformat(
                entities["scheduled_time"].replace("Z", "+00:00")
            )
        except:
    ...
    def list_by_user(
        ...
        if status == "active":
            query = query.filter(
                and_(Reminder.is_active == True, Reminder.is_completed == False)
            )
    ...
    def get_upcoming(
        ...
        ).order_by(Reminder.actual_reminder_time.asc()).all()
    ...
    def count_by_user(self, user_id: int) -> dict:
        ...
        Returns:
            Dict com contagens
        """
        total = self.db.query(func.count(Reminder.id)).filter(
            Reminder.user_id == user_id
        ).scalar()

        active = self.db.query(func.count(Reminder.id)).filter(
            and_(
                Reminder.user_id == user_id,
                Reminder.is_active == True,
                Reminder.is_completed == False
        ...
        completed = self.db.query(func.count(Reminder.id)).filter(
            and_(
                Reminder.user_id == user_id,
                Reminder.is_completed == True
            )
